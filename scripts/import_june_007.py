"""
Import Форма 007 — червень 2026 з data/007/06_data.xlsx.
Кожен лист ('01'-'30') = один день. Рядки даних 12-37 (депт-рядки + КНП/Пологовий/ВСЬОГО).
"""
import sys
import os
...
XLSX_PATH = os.path.join(os.path.dirname(os.path.dirname(__file__)), 'data/007/06_data.xlsx')
YEAR, MONTH = 2026, 6
...
with app.app_context():
    ...
    print(f"\nDailyReport за червень 2026: {total}")
    print(f"Очікується: ~{30 * len(dept_by_name)} ({len(dept_by_name)} відділень × 30 днів)")
