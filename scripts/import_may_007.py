"""
Import Форма 007 — травень 2026 з data/007/05_data.xlsx.
Кожен лист ('01'-'31') = один день. Рядки даних 12-37 (депт-рядки + КНП/Пологовий/ВСЬОГО).
"""
import sys
import os
...
# xlsx назва відділення → назва відділення в БД (None = навмисно пропущено)
NAME_MAP = {
    ...
    # дитячі підрозділи
    'Хірургічні дитячі':    'Хірургічне дит.',
    'Урологічні діти':      'Урологічне дит.',
    'Травматологіч діти':   'Травматологічне дит.',
    # кардіохірургія (перейменовувалось у файлах)
    'Кардіохірургічні':     'Кардіохірургічне',
    'Кардіохірургічне':     'Кардіохірургічне',
    'Інтервенція':          'Кардіохірургічне',
    # підсумкові рядки
    'КНП "Калуська ЦРЛ"':  None,
    'ВСЬОГО по КНП':        None,
    'Готувала:  Луців Л':   None,
}

XLSX_PATH = os.path.join(os.path.dirname(os.path.dirname(__file__)), 'data/007/05_data.xlsx')
YEAR, MONTH = 2026, 5
...
with app.app_context():
    ...
    # Оновлюємо row_no з листа '01' (стабільний для всіх листів)
    ws0 = wb['01']
    row_no_updated = 0
    ...
    for sheet_name in wb.sheetnames:
        ...
        for row in ws.iter_rows(min_row=12, max_row=37, values_only=True):
            ...
            CHILDREN_DEPTS = {'Педіатричне', 'Хірургічне дит.', 'Урологічне дит.', 'Травматологічне дит.'}
            if dept.name in CHILDREN_DEPTS:
    ...
    total = DailyReport.query.filter(
        ...
        DailyReport.report_date <= date(YEAR, MONTH, 31),
    ).count()
    print(f"\nDailyReport за травень 2026: {total}")
    print(f"Очікується: ~{31 * len(dept_by_name)} ({len(dept_by_name)} відділень × 31 день)")
