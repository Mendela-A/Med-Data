...
- Додано індекси для оптимізації запитів (по даті, статусу, лікарю)
- Створено зв'язки з таблицею `users` (created_by, updated_by)
- Колонки: id, date, nszu_record_id, doctor, status, detail, comment, fakt_summ
...
### Автоматичне виконання міграцій
Міграції виконуються **автоматично** при старті Docker контейнера через `entrypoint.sh`:
- При кожному `docker-compose up` виконується `flask db upgrade`
- Не потрібно запускати міграції вручну

### Ручне виконання міграцій в Docker
Якщо потрібно виконати міграції вручну:
...
- **Обов'язково зберігайте всі файли міграцій в Git** (папка `migrations/`)
- Всі міграції керуються через Flask-Migrate (Alembic)
